...
// section1 box2_1, footer swiper 사진 슬라이드
...
// // section6 이미지 translate

$(window).scroll(function () {
 let scrollTop = $(window).scrollTop();

 if (scrollTop >= $(".section6 .js_contents .con1_container .img_wrap").offset().top - $(window).height()/5) {    
     let word = $(".section6 .js_contents .con1_container .img_wrap");
     let offset = scrollTop - word.offset().top;

     gsap.to(word, {duration: .3, x: offset,  ease: "power2.out"})
 }

 if (scrollTop >= $(".section6 .js_contents .con2_container .img_wrap").offset().top - $(window).height()/5) {    
     let word = $(".section6 .js_contents .con2_container .img_wrap");
     let offset = scrollTop - word.offset().top;
